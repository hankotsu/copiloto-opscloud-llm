...
- **G18, región no verificada (pendiente):** en otra corrida el modelo añadió "distribuidos en la región sa-saopaulo-1" (3 de 3 afirmaciones respaldadas). El verificador no extrae regiones, así que esa frase no se revisó. Comprobar en las `filas` de la tool si los 11 volúmenes están todos en sa-saopaulo-1: ______ (si no, es un caso real de L3/L5: afirmación sin cifra que pasa como VERIFICADA).
